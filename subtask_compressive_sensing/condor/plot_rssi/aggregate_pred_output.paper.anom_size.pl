...
sub plot_anomaly_size {
    ...
    my $cmd = "sed 's/FILE_NAME/pred.$output_file/g; s/FIG_NAME/pred.$output_file/g; s/X_RANGE_S/0/g; s/X_RANGE_E//g; s/Y_RANGE_E/$y_max/g; s/X_LABEL/Anomaly Size k/g; s/Y_LABEL/NMAE/g; ' $plot_file > tmp.plot.pred.plot";
    `$cmd`;
    ...
    if($drop_mode eq "ind") {
        ...
        # `$cmd`;
        my $cmd = "sed 's/FILE_NAME/dect.$output_file/g; s/FIG_NAME/dect.$output_file/g; s/X_RANGE_S/2/g; s/X_RANGE_E//g; s/Y_RANGE_E/1/g; s/X_LABEL/Anomaly Size k/g; s/Y_LABEL/F1-Score/g; ' plot.pred.mother.plot > tmp.plot.dect.plot";
        `$cmd`;
