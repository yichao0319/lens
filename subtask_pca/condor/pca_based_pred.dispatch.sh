            ...
            for opt_swap_mat in ${opt_swap_mats[@]}; do
                for rank in ${ranks[@]}; do
                    for block_size in ${block_sizes[@]}; do
                        # echo ${block_ws[$block_size]}
                        # echo ${block_hs[$block_size]}

                        echo ${func}.${filename}.${num_frames}.${width}.${height}.${block_ws[$block_size]}.${block_hs[$block_size]}.${rank}.${opt_swap_mat}.${loss_rate}.${seed}
                        sed "s/INPUT_DIR/${input_dir}/g; s/FILENAME/${filename}/g;s/NUM_FRAMES/${num_frames}/g;s/BLOCK_HEIGHT/${block_hs[$block_size]}/g;s/BLOCK_WIDTH/${block_ws[$block_size]}/g;s/WIDTH/${width}/g;s/HEIGHT/${height}/g;s/RANK/${rank}/g;s/OPT_SWAP_MAT/${opt_swap_mat}/g;s/LOSS_RATE/${loss_rate}/g;s/SEED/${seed}/g" ${func}.mother.sh > tmp.${func}.${filename}.${num_frames}.${width}.${height}.${block_ws[$block_size]}.${block_hs[$block_size]}.${rank}.${opt_swap_mat}.${loss_rate}.${seed}.sh
                        sed "s/XXX/${filename}.${num_frames}.${width}.${height}.${block_ws[$block_size]}.${block_hs[$block_size]}.${rank}.${opt_swap_mat}.${loss_rate}.${seed}/g" ${func}.mother.condor > tmp.${func}.${filename}.${num_frames}.${width}.${height}.${block_ws[$block_size]}.${block_hs[$block_size]}.${rank}.${opt_swap_mat}.${loss_rate}.${seed}.condor
                        condor_submit tmp.${func}.${filename}.${num_frames}.${width}.${height}.${block_ws[$block_size]}.${block_hs[$block_size]}.${rank}.${opt_swap_mat}.${loss_rate}.${seed}.condor
                    done
                done
            done
        done
