                    ...
                    for my $opt_scope (@opt_scopes) {
                        ...
                        for my $opt_sel_method (@opt_sel_methods) {
                            ...
                            for my $num_sel_block (@num_sel_blocks) {
                                ...
                                my @ccs;
                                for my $seed (@seeds) {
                                    my $this_file_name = "$input_dir/$func.$file_name.$num_frames.$width.$height.$block_size.$block_size.$num_sel_block.$opt_delta.$opt_scope.$opt_sel_method.$opt_swap_mat.$drop_rate.$seed.txt";
                                    ...
                                    while(<FH>) {
                                        ...
                                        my $buf = "$file_name, $num_frames, $width, $height, $block_size, $block_size, $num_sel_block, $opt_delta, $opt_scope, $opt_sel_method, $opt_swap_mat, $drop_rate, $seed, $mse, $mae, $cc\n";
                                        print $buf;
                                        print FH_OUT $buf;
                                    }
                                } ## end seeds
...
foreach my $trace (sort {$a cmp $b} (keys %{ $best{TRACE} })) {
    foreach my $drop_rate (sort {$a <=> $b} (keys %{ $best{TRACE}{$trace}{DROP_RATE} })) {
        ...
        foreach my $opt_sel_method (sort {$a <=> $b} (keys %{ $best{TRACE}{$trace}{DROP_RATE}{$drop_rate}{OPT_SEL_METHOD} })) {
            print "$trace (drop $drop_rate), opt_sel_method=$opt_sel_method, ".$best{TRACE}{$trace}{DROP_RATE}{$drop_rate}{OPT_SEL_METHOD}{$opt_sel_method}{MSE}."\n";
        }
        ...
        foreach my $opt_sel_method (sort {$a <=> $b} (keys %{ $best{TRACE}{$trace}{DROP_RATE}{$drop_rate}{OPT_SEL_METHOD} })) {
            print "$trace (drop $drop_rate), opt_sel_method=$opt_sel_method, ".$best{TRACE}{$trace}{DROP_RATE}{$drop_rate}{OPT_SEL_METHOD}{$opt_sel_method}{MAE}."\n";
        }
        foreach my $num_sel_block (sort {$a <=> $b} (keys %{ $best{TRACE}{$trace}{DROP_RATE}{$drop_rate}{NUM_SEL_BLOCK} })) {
        ...
        foreach my $opt_sel_method (sort {$a <=> $b} (keys %{ $best{TRACE}{$trace}{DROP_RATE}{$drop_rate}{OPT_SEL_METHOD} })) {
            print "$trace (drop $drop_rate), opt_sel_method=$opt_sel_method, ".$best{TRACE}{$trace}{DROP_RATE}{$drop_rate}{OPT_SEL_METHOD}{$opt_sel_method}{CC}."\n";
        }
        foreach my $num_sel_block (sort {$a <=> $b} (keys %{ $best{TRACE}{$trace}{DROP_RATE}{$drop_rate}{NUM_SEL_BLOCK} })) {
