        ...
        for my $opt_dect (3) {
            ...
            for my $opt_delta (1) {
                ...
                for my $opt_fb (18 .. 26) {
                    ...
                    ## gnuplot - data
                    open FH_OUT_2, "> $output_dir/$func.$file_name.exp$expnum..$opt_dect.$opt_delta.$opt_fb.txt" or die $!;

                    ## gnuplot - RP
                    my $cmd = "sed 's/FILENAME/$func.$file_name.exp$expnum..$opt_dect.$opt_delta.$opt_fb/g;s/FIGNAME/$func.$file_name.exp$expnum..$opt_dect.$opt_delta.$opt_fb/g;s/X_RANGE_S/0/g;s/X_RANGE_E/1/g;s/Y_RANGE_S/0/g;s/Y_RANGE_E/1/g;s/X_LABEL/Precision/g;s/Y_LABEL/Recall/g;s/DEGREE/0/g;' $gnuplot_mother.mother.plot > tmp.$gnuplot_mother.$func.$file_name.exp$expnum..$opt_dect.$opt_delta.$opt_fb.plot";
                    `$cmd`;
                    open FH_GNU, ">> tmp.$gnuplot_mother.$func.$file_name.exp$expnum..$opt_dect.$opt_delta.$opt_fb.plot" or die $!;

                    ## gnuplot - F1
                    my $cmd = "sed 's/FILENAME/$func.$file_name.exp$expnum..$opt_dect.$opt_delta.$opt_fb/g;s/FIGNAME/$func.$file_name.exp$expnum..$opt_dect.$opt_delta.$opt_fb.f1/g;s/X_RANGE_S//g;s/X_RANGE_E//g;s/Y_RANGE_S//g;s/Y_RANGE_E//g;s/X_LABEL/threshold/g;s/Y_LABEL/F1-Score/g;s/DEGREE/-45/g;' $gnuplot_mother.mother.plot > tmp.$gnuplot_mother.$func.$file_name.exp$expnum..$opt_dect.$opt_delta.$opt_fb.f1.plot";
                    `$cmd`;
                    open FH_GNU_F1, ">> tmp.$gnuplot_mother.$func.$file_name.exp$expnum..$opt_dect.$opt_delta.$opt_fb.f1.plot" or die $!;
                    ...
                    for my $thresh (5, 10, 15, 20, 30, 50, 70, 100, 150, 200, 250) {
                        ...
                        for my $block_size (30) {
                            ...
                            my $this_file_name = "$input_dir/$func.$file_name.exp$expnum..$num_frames.$width.$height.$block_size.$block_size.$thresh.$opt_dect.$opt_delta.$opt_fb.txt";
                            ...
                            while(<FH>) {
                                ...
                                $data{TRACE}{"$file_name.exp$expnum."}{OPT_DECT}{$opt_dect}{OPT_DELTA}{$opt_delta}{OPT_FB}{$opt_fb}{BLOCK_SIZE}{"$block_size,$block_size"}{THRESH}{$thresh}{TP} = $tp;
                                $data{TRACE}{"$file_name.exp$expnum."}{OPT_DECT}{$opt_dect}{OPT_DELTA}{$opt_delta}{OPT_FB}{$opt_fb}{BLOCK_SIZE}{"$block_size,$block_size"}{THRESH}{$thresh}{TN} = $tn;
                                $data{TRACE}{"$file_name.exp$expnum."}{OPT_DECT}{$opt_dect}{OPT_DELTA}{$opt_delta}{OPT_FB}{$opt_fb}{BLOCK_SIZE}{"$block_size,$block_size"}{THRESH}{$thresh}{FP} = $fp;
                                $data{TRACE}{"$file_name.exp$expnum."}{OPT_DECT}{$opt_dect}{OPT_DELTA}{$opt_delta}{OPT_FB}{$opt_fb}{BLOCK_SIZE}{"$block_size,$block_size"}{THRESH}{$thresh}{FN} = $fn;

                                $data{TRACE}{"$file_name.exp$expnum."}{OPT_DECT}{$opt_dect}{OPT_DELTA}{$opt_delta}{OPT_FB}{$opt_fb}{BLOCK_SIZE}{"$block_size,$block_size"}{THRESH}{$thresh}{PRECISION} = $precision;
                                $data{TRACE}{"$file_name.exp$expnum."}{OPT_DECT}{$opt_dect}{OPT_DELTA}{$opt_delta}{OPT_FB}{$opt_fb}{BLOCK_SIZE}{"$block_size,$block_size"}{THRESH}{$thresh}{RECALL} = $recall;
                                $data{TRACE}{"$file_name.exp$expnum."}{OPT_DECT}{$opt_dect}{OPT_DELTA}{$opt_delta}{OPT_FB}{$opt_fb}{BLOCK_SIZE}{"$block_size,$block_size"}{THRESH}{$thresh}{F1SCORE} = $f1score;

                                my $buf = "$file_name, $expnum, $num_frames, $width, $height, $opt_dect, $opt_delta, $block_size, $block_size, $thresh, $tp, $tn, $fp, $fn, $precision, $recall, $f1score\n";
                                print $buf;
                                print FH_OUT $buf;
                    ...
                    ## gnuplot
                    $cmd = "gnuplot tmp.$gnuplot_mother.$func.$file_name.exp$expnum..$opt_dect.$opt_delta.$opt_fb.plot";
                    `$cmd`;

                    $cmd = "gnuplot tmp.$gnuplot_mother.$func.$file_name.exp$expnum..$opt_dect.$opt_delta.$opt_fb.f1.plot";
                    `$cmd`;
                }  ## end of opt_fb
            }  ## end of opt_delta
        }
